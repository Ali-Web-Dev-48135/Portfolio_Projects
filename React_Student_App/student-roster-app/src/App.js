...
const App = () => {
  ...
  const deleteStudent = (studentId) => {
    axios
      ...
      .then((response) => {
        setStudentsArray((oldArray) =>
          oldArray.filter((element) => element.studentId !== studentId)
        );
      })
      .catch((error) => console.log(error));
  };
  ...
  const updateStudent = (updatedStudentData, studentId) => {
    ...
    axios
      .put(`/students/${studentId}`, updatedStudentData)
      .then((response) => {
        setStudentsArray(response.data);
      })
      .catch((error) => console.log(error));
  ...
  const editStudent = (editStudentId) => {
    ...
    let editToDoItem = temporaryStudentArray.findIndex(
      (element) => element.studentId === editStudentId
    );
    temporaryStudentArray[editToDoItem].isEditing =
      !temporaryStudentArray[editToDoItem].isEditing;
      ...
      <BrowserRouter>
        <StudentMain>
          <Routes>
            ...
            <Route
              path="getallstudents"
              element={<StudentGetAll studentList={studentsArray} />}
            />
            <Route
              path="/addstudent"
